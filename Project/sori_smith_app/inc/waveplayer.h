...
uint32_t Audio2FlashPlay(uint16_t* pBuffer, uint32_t FullSize, uint32_t StartAdd);

int WavePlayerInit(uint32_t AudioFreq);
void WavePlayerStop(void);
void WavePlayerPauseResume(uint8_t state);
void WavePlayerPauseResume2(uint8_t state);
uint8_t WaveplayerCtrlVolume(uint8_t volume);
uint8_t WaveplayerCtrlVolume2(uint8_t volume);

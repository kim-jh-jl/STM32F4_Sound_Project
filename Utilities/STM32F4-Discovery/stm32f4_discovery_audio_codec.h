...
/** @defgroup STM32F4_DISCOVERY_AUDIO_CODEC_Exported_Macros
  * @{
  */ 
//#define VOLUME_CONVERT(x)    ((Volume > 100)? 100:((uint8_t)(((Volume+9) * 255) / 100)))
#define VOLUME_CONVERT(x)    ((Volume > 30)? ((uint8_t)(30+(Volume-30)*3)):((uint8_t)(Volume)))
...
uint32_t EVAL_AUDIO2_Play(uint16_t* pBuffer, uint32_t Size);
uint32_t EVAL_AUDIO2_PauseResume(uint32_t Cmd);
uint32_t EVAL_AUDIO2_Stop(uint32_t CodecPowerDown_Mode);
uint32_t EVAL_AUDIO2_VolumeCtl(uint8_t Volume);
uint32_t EVAL_AUDIO2_Mute(uint32_t Command);
void Audio2_MAL_Play(uint32_t Addr, uint32_t Size);
